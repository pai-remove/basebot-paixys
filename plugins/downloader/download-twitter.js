...
let handler = async (m, { conn, args, text, usedPrefix, command }) => {
    ...
    m.reply("*Mohon tunggu, sedang diproses...*");
    ...
    try {
        ...
        if (result.error) {
            return m.reply(
                "Gagal mengunduh media. Pastikan URL valid dan dapat diakses."
            );
        }
        ...
        if (result.medias && result.medias.length > 0) {
            ...
            const isImage =
                result.type === "photo" ||
                /\.(jpg|jpeg|png|webp)$/i.test(result.url);
...
handler.command = ["twitter", "twiter", "x"];
